...
func NewTransactionManager() *TransactionManager {
	return &TransactionManager{
		twoPhase:      NewTwoPhaseCommit(30 * time.Second),
		threePhase:    NewThreePhaseCommit(),
		distributedLock: NewDistributedLock(10 * time.Second),
	}
}

// CreateTwoPhase 创建两阶段提交事务
func (tm *TransactionManager) CreateTwoPhase() *TwoPhaseCommit {
	return NewTwoPhaseCommit(30 * time.Second)
}

// CreateThreePhase 创建三阶段提交事务
func (tm *TransactionManager) CreateThreePhase() *ThreePhaseCommit {
	return NewThreePhaseCommit()
}

// CreateSaga 创建 Saga 事务
func (tm *TransactionManager) CreateSaga(steps ...*SagaStep) *SagaTransaction {
	return NewSagaTransaction(steps...)
}

// CreateTCC 创建 TCC 事务
func (tm *TransactionManager) CreateTCC() *TCCTransaction {
	return NewTCCTransaction()
}

// GetLock 获取分布式锁
func (tm *TransactionManager) GetLock() *DistributedLock {
	return tm.distributedLock
}
