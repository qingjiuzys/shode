...
- **问题**: SimpleParser不支持变量赋值语法
- **修复**: 
  - 添加 `parseAssignment` 方法识别 `var = value` 语法
  - 修改 `parseCommand` 返回 `types.Node` 支持AssignmentNode
  - 自动去除引号
...
- `pkg/engine/engine.go` - 集成变量展开到命令执行和函数调用
